        ...
        if (!knowledge.isResearchComplete(researchkey) && ResearchManager.doesPlayerHaveRequisites(player, researchkey)) {
            ...
            if ((re = ResearchCategories.getResearch(researchkey)) != null) {
                ...
                if (re.getStages() != null) {
                    ...
                    if (re.getStages().length == 1 && cs == 0 && re.getStages()[0].getCraft() == null && re.getStages()[0].getObtain() == null && re.getStages()[0].getResearch() == null) {
                        ++cs;
                    } else if (re.getStages().length > 1 && re.getStages().length <= cs + 1 && cs < re.getStages().length && re.getStages()[cs].getCraft() == null && re.getStages()[cs].getObtain() == null && re.getStages()[cs].getResearch() == null) {
                        ++cs;
                    }
                ...
                if (popups) {
                    if (sync) {
                        ...
                        if (re.getRewardItem() != null) {
                            ItemStack[] cs = re.getRewardItem();
                            int currentStage = cs.length;
                            for (warp = 0; warp < currentStage; ++warp) {
                                ItemStack rs = cs[warp];
                                if (player.inventory.addItemStackToInventory(rs.copy())) continue;
            ...
            try {
                ...
                for (JsonElement element : entries) {
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        Runic.LOGGER.error("Invalid research entry [" + a + "] found in " + loc.toString());
                        a--;
                    }
                }
