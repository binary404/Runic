...
public class ShaderRenderType extends RenderType {
    ...
    private final ShaderHandler.RunicShader shader;

    @Nullable
    private final ShaderCallback cb;

    public ShaderRenderType(ShaderHandler.RunicShader shader, @Nullable ShaderCallback cb, RenderType delegate) {
        super("runic:" + delegate.toString() + "_with_" + shader.name(), delegate.getVertexFormat(), delegate.getDrawMode(), delegate.getBufferSize(), delegate.isUseDelegate(), true,
                () -> {
