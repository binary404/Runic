...
import binary404.mystica.api.research.ResearchStage;
        ...
        if (research.getStages() != null) {
            ...
            if (p != null) {
                for (int a = 0; a < p.length; a++) {
                    int pp = CapabilityHelper.getKnowledge(player).getResearchPoints(p[a].category);
                    if (pp < p[a].amount)
                        return false;
                }
                for (int a = 0; a < p.length; a++) {
                    ResearchManager.addResearchPoints(player, p[a].category, -p[a].amount * 16);
                }
            }
