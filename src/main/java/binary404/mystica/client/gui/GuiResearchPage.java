...
import binary404.mystica.api.multiblock.Matrix;
import binary404.mystica.api.multiblock.MultiBlockComponent;
import binary404.mystica.api.research.*;
import binary404.mystica.client.core.handler.MultiBlockHandler;
import binary404.mystica.client.gui.research_extras.Page;
import binary404.mystica.client.gui.research_extras.PageImage;
...
import binary404.mystica.common.core.network.research.PacketSyncProgressToServer;
import binary404.mystica.common.core.util.InventoryUtils;
import binary404.mystica.common.core.util.StringUtil;
...
public class GuiResearchPage extends Screen {
    ...
    ResourceLocation tex1 = new ResourceLocation("mystica", "textures/gui/gui_researchbook.png");
    ResourceLocation tex2 = new ResourceLocation("mystica", "textures/gui/gui_researchrock_overlay.png");
    ResourceLocation tex4 = new ResourceLocation("mystica", "textures/gui/stone.png");
    ResourceLocation dummyResearch = new ResourceLocation("mystica", "textures/aspects/_unknown.png");
    ResourceLocation dummyMap = new ResourceLocation("mystica", "textures/research/rd_map.png");
    ResourceLocation dummyFlask = new ResourceLocation("mystica", "textures/research/rd_flask.png");
    ResourceLocation dummyChest = new ResourceLocation("mystica", "textures/research/rd_chest.png");
    int hrx = 0;
    int hry = 0;
    ...
    public boolean mouseClicked(double par1, double par2, int par3) {
        ...
        if (shownRecipe == null && !this.hold && this.hasAllRequisites && mx >= 0 && my >= 0 && mx < 64 && my < 12) {
            PacketHandler.HANDLER.sendToServer(new PacketSyncProgressToServer(this.research.getKey(), false, true, true));
    ...
    private void drawRequirements(MatrixStack stack, int x, int mx, int my, ResearchStage stage) {
        ...
        if (stage.getResearch() != null) {
            ...
            this.drawPopupAt(x - 15, y, mx, my, "mystica.need.research");
            Object loc = null;
            if (this.hasResearch != null) {
                ...
                for (int a = 0; a < stage.getResearch().length; ++a) {
                    ...
                    String text = I18n.format((String) ("mystica.research." + key + ".text"));

                    if (key.startsWith("!")) {
                        String k = key.replaceAll("!", "");
                        Aspect as = (Aspect) AspectRegistry.aspects.get(k);
                        if (as != null) {
                            loc = as;
                            text = as.getName();
                        }
                    ...
                    if (loc instanceof Aspect) {
                        this.minecraft.getTextureManager().bindTexture(((Aspect) loc).getImage());
                        java.awt.Color cc = new java.awt.Color(((Aspect) loc).getColor());
                        RenderSystem.color3f(cc.getRed() / 255F, cc.getGreen() / 255.0F, cc.getBlue() / 255F);
        ...
        if (stage.getObtain() != null) {
            ...
            RenderSystem.color4f((float) 1.0f, (float) 1.0f, (float) 1.0f, (float) 0.25f);
            this.minecraft.getTextureManager().bindTexture(this.tex1);
            this.blit(stack, x - 12, y - 1, 200, 216, 56, 16);
            this.drawPopupAt(x - 15, y, mx, my, "mystica.need.obtain");
            RenderSystem.color4f((float) 1.0f, (float) 1.0f, (float) 1.0f, (float) 1.0f);
        ...
        if (stage.getCraft() != null) {
            ...
            RenderSystem.color4f((float) 1.0f, (float) 1.0f, (float) 1.0f, (float) 0.25f);
            this.minecraft.getTextureManager().bindTexture(this.tex1);
            this.blit(stack, x - 12, y - 1, 200, 200, 56, 16);
            this.drawPopupAt(x - 15, y, mx, my, "mystica.need.craft");
            RenderSystem.color4f((float) 1.0f, (float) 1.0f, (float) 1.0f, (float) 1.0f);
        ...
        if (stage.getPoints() != null) {
            y -= 18;
            b = true;
            int shift = 24;
            RenderSystem.color4f(1.0F, 1.0F, 1.0F, 0.25F);
            this.minecraft.getTextureManager().bindTexture(this.tex1);
            blit(stack, x - 12, y - 1, 200, 184, 56, 16);
            drawPopupAt(x - 15, y, mx, my, "mystica.need.points");
            RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
            if (this.hasKnow != null) {
                if (this.hasKnow.length != (stage.getPoints().length))
                    this.hasKnow = new boolean[(stage.getPoints()).length];
                int ss = 18;
                if ((stage.getPoints()).length > 6)
                    ss = 110 / stage.getPoints().length;
                for (idx = 0; idx < stage.getPoints().length; idx++) {
                    ResearchStage.Point kn = stage.getPoints()[idx];
                    RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
                    stack.push();
                    this.minecraft.getTextureManager().bindTexture(new ResourceLocation("mystica", "textures/research/points.png"));
                    stack.translate((x - 15 + shift), y, 0.0F);
                    stack.scale(0.0625F, 0.0625F, 0.0625F);
                    blit(stack, 0, 0, 0, 0, 255, 255);
                    if (kn.category != null) {
                        this.minecraft.getTextureManager().bindTexture(kn.category.icon);
                        stack.translate(32.0D, 32.0D, 1.0D);
                        stack.push();
                    ...
                    RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
                    stack.push();
                    stack.translate((x - 15 + shift + 16 - this.font.getStringWidth(am) / 2), y + 12, 5.0F);
                    stack.scale(0.5F, 0.5F, 0.5F);
                    this.font.drawStringWithShadow(stack, am, 0.0F, 0.0F, 16777215);
                    stack.pop();
                    if (this.hasKnow[idx]) {
                        stack.push();
                        ...
                        RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
                        this.minecraft.getTextureManager().bindTexture(this.tex1);
                        blit(stack, x - 15 + shift + 8, y, 159, 207, 10, 10);
                        stack.pop();
                    }
                    String s = ResearchCategories.getCategoryName(kn.category.key);
        ...
        if (b) {
            ...
            RenderSystem.color4f((float) 1.0f, (float) 1.0f, (float) 1.0f, (float) 1.0f);
            this.minecraft.getTextureManager().bindTexture(this.tex1);
            this.blit(stack, x + 4, y - 2, 24, 184, 96, 8);
            if (this.hasAllRequisites) {
                this.hrx = x + 20;
                this.hry = y - 6;
                if (this.hold) {
                    String s = I18n.format("mystica.stage.hold");
                    ...
                    String s = I18n.format((String) "mystica.stage.complete");
                    int m = this.minecraft.fontRenderer.getStringWidth(s);
                    this.minecraft.fontRenderer.drawStringWithShadow(stack, s, (float) (x + 52) - (float) m / 2.0f, (float) (y - 4), 16777215);
                }
            }
    ...
    private void parsePages() {
        ...
        for (String s : rawText.split("<IMG>")) {
            int i = s.indexOf("</IMG>");
            if (i < 0) continue;
        ...
        for (String s : firstPassText) {
            List<String> pt1 = StringUtil.listFormattedStringToWidth(s, 140);
            for (String ln : pt1) {
                parsedText.add(ln);
            }
        }
    ...
    private void checkRequisites() {
        if (this.research.getStages() != null) {
            ...
            ResearchStage.Point[] p = stage.getPoints();
            if (p != null) {
                this.hasKnow = new boolean[p.length];
                for (int a = 0; a < p.length; a++) {
                    int pk = this.playerKnowledge.getResearchPoints(p[a].category);
                    if (pk < (p[a].amount)) {
                        this.hasAllRequisites = false;
    ...
    public static class BlueprintBlockAccess implements IBlockReader, IBlockDisplayReader {
        ...
        public int getBlockColor(BlockPos blockPosIn, ColorResolver colorResolverIn) {
            ...
            return colorResolverIn.getColor(world.getBiome(blockPosIn), blockPosIn.getX(), blockPosIn.getZ());
        }
    }
