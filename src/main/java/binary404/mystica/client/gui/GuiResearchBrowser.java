...
import binary404.mystica.common.core.network.research.PacketSyncResearchFlagsToServer;
...
public class GuiResearchBrowser extends Screen {
    ...
    ResourceLocation tx1 = new ResourceLocation("mystica", "textures/gui/gui_research_browser.png");

    @OnlyIn(Dist.CLIENT)
    public GuiResearchBrowser() {
        super(new StringTextComponent("mystica_compendium"));
    ...
    public void updateResearch() {
        ...
        for (String rcl : ResearchCategories.researchCategories.keySet()) {
            ...
            if (top) {
                ...
                addButton(new GuiCategoryButton(rc, v, rcl, false, 10 + this.categoriesBase.size() * 24, 1, 16, 16, I18n.format("mystica.research_category." + rcl), this::category));
                top = false;
            } else if (!top) {
                this.categoriesBase.add(rcl);
                addButton(new GuiCategoryButton(rc, v, rcl, true, 10 + this.categoriesBase.size() * 24, this.height - 17, 16, 15, I18n.format("mystica.research_category." + rcl), this::category));
                top = true;
            }
            ++count;
        }
    ...
    public boolean mouseClicked(double mx, double my, int par3) {
        ...
        if (this.currentHighlight != null && !CapabilityHelper.knowsResearch(this.player, this.currentHighlight.getKey()) && this.canUnlockResearch(this.currentHighlight)) {
            this.updateResearch();
            PacketHandler.HANDLER.sendToServer(new PacketSyncProgressToServer(this.currentHighlight.getKey(), true));
            ...
            this.popupmessage = I18n.format("mystica.research.popup");
            ...
            PacketHandler.HANDLER.sendToServer(new PacketSyncResearchFlagsToServer(this.minecraft.player, this.currentHighlight.getKey()));
            ...
            if (stage > 1 && stage >= this.currentHighlight.getStages().length) {
                PacketHandler.HANDLER.sendToServer(new PacketSyncProgressToServer(this.currentHighlight.getKey(), false, true, false));
            }
            this.minecraft.displayGuiScreen(new GuiResearchPage(this.currentHighlight, null, this.guiMapX, this.guiMapY));
        }
    ...
    protected void genResearchBackgroundZoomable(MatrixStack stack, int mx, int my, int locX, int locY) {
        ...
        for (int var24 = 0; var24 < this.researchEntries.size(); ++var24) {
            ...
            if (iconResearch.hasMeta(ResearchEntry.EnumResearchMeta.ROUND)) {
                this.blit(stack, iconX - 8, iconY - 8, 144, 48 + (iconResearch.hasMeta(ResearchEntry.EnumResearchMeta.HIDDEN) ? 32 : 0), 32, 32);
            } else {
                int ix = 80;
                int iy = 48;
            ...
            if (iconResearch.hasMeta(ResearchEntry.EnumResearchMeta.SPIKY)) {
                blit(stack, iconX - 8, iconY - 8, 176, 48 + (iconResearch.hasMeta(ResearchEntry.EnumResearchMeta.HIDDEN) ? 32 : 0), 32, 32);
            }
            boolean bw = false;
        ...
        if (this.currentHighlight != null) {
            ...
            if (this.canUnlockResearch(this.currentHighlight)) {
                if (!CapabilityHelper.getKnowledge(this.player).isResearchComplete(this.currentHighlight.getKey()) && this.currentHighlight.getStages() != null) {
                    ...
                    if (stage > 0) {
                        text.add("@@" + (Object) TextFormatting.AQUA + I18n.format((String) "mystica.research.stage") + " " + stage + "/" + this.currentHighlight.getStages().length + (Object) TextFormatting.RESET);
                    } else {
                        text.add("@@" + (Object) TextFormatting.GREEN + I18n.format((String) "mystica.research.begin") + (Object) TextFormatting.RESET);
                    }
                }
            } else {
                text.add("@@\u00a7c" + I18n.format("mystica.researchmissing"));
                int a = 0;
            ...
            if (CapabilityHelper.getKnowledge(this.player).hasResearchFlag(this.currentHighlight.getKey(), IPlayerKnowledge.EnumResearchFlag.RESEARCH)) {
                text.add(I18n.format("mystica.research.newresearch"));
            }
            if (CapabilityHelper.getKnowledge(this.player).hasResearchFlag(this.currentHighlight.getKey(), IPlayerKnowledge.EnumResearchFlag.PAGE)) {
                text.add(I18n.format((String) "mystica.research.newpage"));
            }
    ...
    private class GuiCategoryButton extends Button {
        ...
        public GuiCategoryButton(ResearchCategory rc, int completion, String key, boolean flip, int x, int y, int w, int h, String text, IPressable onPress) {
            super(x, y, w, h, ITextComponent.getTextComponentOrEmpty(text), onPress);
            this.rc = rc;
            this.key = key;
            this.flip = flip;
        ...
        public void renderButton(MatrixStack stack, int mouseX, int mouseY, float p_render_3_) {
            if (this.visible) {
                ...
                if (this.isHovered) {
                    ...
                    int t = 9;
                    if (nr) {
                        this.drawString(stack, minecraft.fontRenderer, I18n.format((String) "mystica.research.newresearch"), !this.flip ? this.x + 22 : GuiResearchBrowser.this.screenX + 9 - minecraft.fontRenderer.getStringWidth(I18n.format((String) "mystica.research.newresearch")), this.y + 4 + t + GuiResearchBrowser.this.addonShift, 16777215);
                        t += 9;
                    }
                    if (np) {
                        this.drawString(stack, minecraft.fontRenderer, I18n.format((String) "mystica.research.newpage"), !this.flip ? this.x + 22 : GuiResearchBrowser.this.screenX + 9 - minecraft.fontRenderer.getStringWidth(I18n.format((String) "mystica.research.newpage")), this.y + 4 + t + GuiResearchBrowser.this.addonShift, 16777215);
                    }
                }
            }
        }
