...
public class RenderingUtils {
    ...
    public static final VertexFormat VERTEXFORMAT_POS_TEX_CO_LM_NO = (new VertexFormat(ImmutableList.<VertexFormatElement>builder()
    ...
    public static void drawTexturedQuad(float par1, float par2, float par3, float par4, float par5, float par6, float zLevel, float r, float g, float b, float a, IVertexBuilder buffer, MatrixStack stack) {
        float var7 = 0.00390625F;
        float var8 = 0.00390625F;
        Matrix4f m = stack.getLast().getMatrix();
        buffer.pos(m, (par1 + 0.0F), (par2 + par6), zLevel).tex(((par3 + 0.0F) * var7), ((par4 + par6) * var8)).color(r, g, b, a).endVertex();
        buffer.pos(m, (par1 + par5), (par2 + par6), zLevel).tex(((par3 + par5) * var7), ((par4 + par6) * var8)).color(r, g, b, a).endVertex();
        buffer.pos(m, (par1 + par5), (par2 + 0.0F), zLevel).tex(((par3 + par5) * var7), ((par4 + 0.0F) * var8)).color(r, g, b, a).endVertex();
        buffer.pos(m, (par1 + 0.0F), (par2 + 0.0F), zLevel).tex(((par3 + 0.0F) * var7), ((par4 + 0.0F) * var8)).color(r, g, b, a).endVertex();
    }

    public static void drawTexturedQuad(float par1, float par2, float par3, float par4, float par5, float par6, float zLevel) {
        float var7 = 0.00390625F;
        float var8 = 0.00390625F;
        Tessellator var9 = Tessellator.getInstance();
        var9.getBuffer().begin(7, DefaultVertexFormats.POSITION_TEX);
        ...
        if (!textList.isEmpty()) {
            ...
            if (b) {
                ...
                for (Object o : textList) {
                    ...
                    List tl2 = fr.trimStringToWidth(ITextProperties.func_240652_a_(textLine2), textLine2.startsWith("@@") ? (max * 2) : max);
                    for (Object o2 : tl2) {
    ...
    public static void drawRect(Matrix4f matrix, IVertexBuilder buffer, int frame, int gridSize) {
        drawRect(matrix, buffer, frame, gridSize, 1.0F, 1.0F, 1.0F, 1.0F);
    }

    public static void drawRect(Matrix4f matrix, IVertexBuilder buffer, int frame, int gridSize, float r, float g, float b, float a) {
        int gridX = frame % gridSize;
        int gridY = frame / gridSize;

        float minU = (float) gridX / (float) gridSize;
        float maxU = minU + 1.0F / (float) gridSize;
        float minV = (float) gridY / (float) gridSize;
        float maxV = minV + 1.0F / (float) gridSize;

        buffer.pos(matrix, 0.0F - 0.5F, 0.0F - 0.5F, 0.0F).color(r, g, b, a).tex(minU, maxV).endVertex();
        buffer.pos(matrix, 0.5F, 0.0F - 0.5F, 0.0F).color(r, g, b, a).tex(maxU, maxV).endVertex();
        buffer.pos(matrix, 0.5F, 0.5F, 0.0F).color(r, g, b, a).tex(maxU, minV).endVertex();
        buffer.pos(matrix, 0.0F - 0.5F, 0.5F, 0.0F).color(r, g, b, a).tex(minU, minV).endVertex();
    }
